...
export default function PlayLoginPage() {
  ...
  useEffect(() => {
    ...
    if (!k) {
      setStatus('error');
      setErrorMsg('Invalid link. Please open the Mini App again to get a fresh link.');
      return;
    }

    const attemptLogin = async () => {
      try {
        const res = await axios.get(`${API_URL}/api/auth/play-login?k=${encodeURIComponent(k)}`);
      ...
      {status === 'error' && (
        <div style={{ textAlign: 'center', animation: 'fadeIn 0.3s ease', maxWidth: '320px' }}>
          ...
          <div style={{
            ...
            Go back to the Mini App and tap<br />
            <strong>"🌐 PLAY IN BROWSER"</strong><br />

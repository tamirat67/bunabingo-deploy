...
// ─────────────────────────────────────────────────────────────────────────────
// ADMIN: Start a Public Game (cross-agent, commission goes to platform)
// POST /api/admin/games/start-public
// Body: { roomType: 'CASUAL' | 'STANDARD' | 'PRO' | 'VIP' | 'JACKPOT' }
// ─────────────────────────────────────────────────────────────────────────────
router.post('/admin/games/start-public', telegramAuthMiddleware, adminMiddleware, async (req: Request, res: Response) => {
  try {
    const { roomType } = req.body;
    if (!roomType) return res.status(400).json({ error: 'roomType is required' });

    const validTypes = ['CASUAL', 'STANDARD', 'PRO', 'VIP', 'JACKPOT'];
    if (!validTypes.includes(roomType)) {
      return res.status(400).json({ error: `Invalid roomType. Must be one of: ${validTypes.join(', ')}` });
    }

    // Find the room
    const room = await prisma.room.findFirst({ where: { type: roomType as any } });
    if (!room) return res.status(404).json({ error: `Room ${roomType} not found` });

    // Create a new public waiting game
    ...
    // Trigger the game engine countdown
    const { startCountdown } = await import('../game/engine');
    setImmediate(async () => {
      try {
        await startCountdown(game.id, 0);
      } catch (e) {
        logger.error(`[PublicGame] Countdown start failed for game ${game.id}:`, e);
      }
    });

    logger.info(`[PublicGame] Admin started public game ${game.id} in room ${roomType}`);
    res.json({ success: true, gameId: game.id, roomType, isPublic: true });
  } catch (err: any) {
    logger.error('[PublicGame] Failed to start public game:', err);
    res.status(500).json({ error: err.message || 'Failed to start public game' });
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// PUBLIC: Get active public games (for bot invite link generation)
// GET /api/games/public
// ─────────────────────────────────────────────────────────────────────────────
router.get('/games/public', async (_req: Request, res: Response) => {
  try {
    const games = await prisma.game.findMany({
      where: {
        isPublic: true,

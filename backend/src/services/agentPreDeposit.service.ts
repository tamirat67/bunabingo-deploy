...
export async function debitAgentCommissionForGame(
  ...
  // 4. HARD-BLOCK if agent pre-deposit balance is insufficient.
  //    Game is NOT allowed until agent tops up their pre-deposit wallet.
  if (balance.lessThan(commission)) {
    ...
    const msg =
      `Agent pre-deposit balance insufficient for ${agentName}. ` +
      `Required: ${commission.toFixed(2)} ETB, ` +
      `Available: ${balance.toFixed(2)} ETB. ` +
      `Agent must top up their pre-deposit wallet before games can start.`;
  ...
  await prisma.$transaction(async (tx) => {
    await tx.agentPreDepositWallet.update({
      ...
      data: {
        balance: newBalance,
        totalDebited: new Decimal(wallet.totalDebited?.toString() ?? 0).add(commission),
        updatedAt: new Date(),
      },
    });

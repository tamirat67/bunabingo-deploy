...
exports.aviatorEnterRoom = aviatorEnterRoom;
exports.aviatorPlaceBet = aviatorPlaceBet;
exports.aviatorCashOut = aviatorCashOut;
exports.startAviatorLoop = startAviatorLoop;
...
function phaseToGameState(phase) {
    switch (phase) {
...
async function runCountdown() {
    ...
    // Create DB record
    const game = await prisma_1.default.aviatorGame.create({
        data: { status: 'WAITING' },
    });
    gameState.gameId = game.id;
    gameState.phase = 'COUNTDOWN';
    gameState.crashAt = generateCrash();
    logger_1.logger.info(`[Aviator] New game ${game.id} | crashAt=${gameState.crashAt}`);
    broadcastState();
...
async function runFlying() {
    if (!gameState.gameId)
        return;
    await prisma_1.default.aviatorGame.update({
...
async function runCrash() {
    ...
    await prisma_1.default.aviatorGame.update({
        ...
        data: {
            status: 'FINISHED',
            crashMultiplier: new library_1.Decimal(gameState.crashAt.toFixed(2)),
            finishedAt: new Date(),
        },
    });
    ...
    try {
        ...
        const masterAgent = await prisma_1.default.user.findFirst({
            where: {
                OR: [
                    { username: "Luel1616" },
        ...
        if (masterAgent) {
            ...
            logger_1.logger.info(`[Aviator] Deposited ${profit} ETB profit to Master Agent @Luel1616`);
        }
        else {
            logger_1.logger.warn(`[Aviator] Master Agent @Luel1616 not found. Profit of ${profit} ETB was not deposited.`);
        }
    }
    catch (err) {
        logger_1.logger.error(`[Aviator] Error depositing company profit: ${err.message}`);
    }
    ...
    logger_1.logger.info(`[Aviator] Game ${gameState.gameId} CRASHED at ${gameState.crashAt}×. Lost: ${lostUserIds.length} players.`);
...
async function gameLoop() {
    logger_1.logger.info('[Aviator] Game loop started');
    while (true) {
        try {
            await runWaiting();
            await runCountdown();
            ...
            logger_1.logger.error('[Aviator] Game loop error:', err.message);
            await sleep(3000);
        }
    }
}
 ...
 */
async function aviatorEnterRoom(socketId, userId, io) {
    ...
    try {
        ...
        socket.emit('history', recentBets.map(b => parseFloat((b.game?.crashMultiplier ?? new library_1.Decimal(1)).toString())));
        ...
        logger_1.logger.warn(`[Aviator] enterRoom failed for user ${userId}: ${err.message}`);
    }
}
/**
 * Place a bet for a user (called during COUNTDOWN / BET phase)
 */
async function aviatorPlaceBet(userId, betAmount, target, type) {
    ...
    try {
        ...
        await (0, wallet_service_1.debitWallet)(dbUserId, betAmount, 'TICKET_PURCHASE', gameState.gameId, 'Aviator bet');
        const bet = await prisma_1.default.aviatorBet.create({
            data: {
                gameId: gameState.gameId,
                userId: dbUserId,
                betAmount: new library_1.Decimal(betAmount.toFixed(2)),
                targetMultiplier: target > 1 ? new library_1.Decimal(target.toFixed(2)) : null,
                status: 'PENDING',
            },
        });
        const activeBet = {
        ...
        logger_1.logger.error(`[Aviator] Bet failed for user ${userId}: ${err.message}`);
 ...
 */
async function aviatorCashOut(userId, at, index) {
...
function startAviatorLoop() {
    gameLoop().catch(err => logger_1.logger.error('[Aviator] Fatal loop error:', err));
}
// ── Utilities ─────────────────────────────────────────────────────────────────
function sleep(ms) {
